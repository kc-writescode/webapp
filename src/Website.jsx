...
import { BookOpen, TrendingUp, Users, MessageCircle, Download, FileText, Calculator, CreditCard, ArrowRight, ExternalLink, ChevronRight, ThumbsUp, ThumbsDown, Send, LogOut, Plus, MessageSquare, X } from 'lucide-react';

// Top Header Component (shows logo and user)
const TopHeader = ({ isScrolled, scrollToSection, user }) => (
  <header
...
const FloatingNavBar = ({ navigation, activeSection, scrollToSection }) => (
...
const AuthModal = ({ setShowAuthModal, authMode, setAuthMode, authUsername, setAuthUsername, authPassword, setAuthPassword, handleAuth }) => (
  <div
  ...
  >
    ...
    >
      ...
      <div className="space-y-4">
        <div>
          <label htmlFor="auth-username" className="block text-sm font-semibold text-gray-300 mb-2">Username</label>
          <input
            id="auth-username"
            type="text"
            value={authUsername}
        ...
        <div>
          <label htmlFor="auth-password" className="block text-sm font-semibold text-gray-300 mb-2">Password</label>
          <input
            id="auth-password"
            type="password"
            value={authPassword}
...
const CommunityPage = ({ scrollToSection, user, setUser, setShowAuthModal, setAuthMode, posts, handleVote, showComments, setShowComments }) => (
            ...
            <div className="p-6">
              ...
              {showComments === post.id && (
                <div className="mt-6 pt-6 border-t border-slate-700">
                  ...
                  <div className="space-y-4">
                    {[1, 2].map(i => (
                      <div key={`comment-${post.id}-${i}`} className="flex gap-3 pl-6 border-l-2 border-slate-700">
...
const TrendsPage = ({ scrollToSection, newsArticles }) => (
  <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 text-white pt-24 pb-32 px-4">
    <div className="max-w-6xl mx-auto">
      <button
        onClick={() => scrollToSection('home')}
      ...
      <div className="grid gap-6">
        {newsArticles.map((article) => (
          <div key={article.title} className="bg-slate-800/50 backdrop-blur-sm rounded-2xl border border-slate-700 hover:border-blue-500 transition-all duration-300 p-6 group cursor-pointer">
...
const FinancialWebsite = () => {
  ...
  if (currentPage === 'trends') {
    return (
      <>
        <TopHeader isScrolled={isScrolled} scrollToSection={scrollToSection} user={user} />
        <FloatingNavBar navigation={navigation} activeSection={activeSection} scrollToSection={scrollToSection} />
        <TrendsPage scrollToSection={scrollToSection} newsArticles={newsArticles} />
      </>
    );
  }
